...
use syn::{token, punctuated::{Punctuated, Pair}, Token, parse::{Result, Parse, ParseStream}, parenthesized, braced, Ident};
...
impl ItemId {
    pub fn formatted(&self) -> Option<Ident> {
        match self {
            ...
            Self::Unused(id) => {
                id.map(|id| format_ident!("_unused_{}_", id))
            }

            Self::Let(id) => {
                Some(format_ident!("__{}__", id))
            }
        }
    }
}
        ...
        while !input.is_empty() {
            if input.peek(token::Bracket) || input.peek(token::Paren) {
                ...
                match &id {
                    ...
                    FieldId::Id(id) => {
                        map.insert(Ident::new(&id.to_string(), Span::call_site()), field.r#type.to_owned());
                    }
                }
