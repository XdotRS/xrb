...
impl ParseWithContext for Request {
		...
		Self: Sized,
	{
		...
		if content.peek(Token![,]) {
			...
			if let Ok(r#type) = fork.parse::<Type>() && !fork.peek(Token![,]) {
				// If the tokens following the comma can be parsed as a type,
				// and there is no comma after that type, then this is the type
				// representing the errors generated by the request.

				content.advance_to(fork);
				other_errors = Some((comma, r#type));
			} else {
				// Otherwise, if the tokens following the comma couldn't be
				// parsed as a type, or there was a comma after those tokens,
				// then we parse those tokens as the minor opcode.
				minor_opcode = Some((comma, content.parse::<Expr>()?));

				// If there is a comma following the minor opcode, then we know
				// it must be for the errors generated by the request.
				if content.peek(Token![,]) {
					other_errors = Some((content.parse::<Token![,]>()?, content.parse::<Type>()?));
				}
			}
		}
